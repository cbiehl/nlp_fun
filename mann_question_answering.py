...
D = 50 # word vector dimensionality
learning_rate = 0.005
input_p, output_p = 0.5, 0.5 # dropout
batch_size = 128
passes = 4 # number of passes in episodic memory
ff_hidden_size = 256
...
def download_data():
    ...
    if(not os.path.isfile(glove_zip_file) and not os.path.isfile(glove_vectors_file)):
        ...
        print("Done with downloading Glove embeddings")

    if(not os.path.isfile(data_set_zip) and not (os.path.isfile(train_set_file) and os.path.isfile(test_set_file))):
        print("Downloading data set...")
        urlretrieve("https://s3.amazonaws.com/text-datasets/babi_tasks_1-20_v1-2.tar.gz", data_set_zip)
        print("Done with downloading dataset")
...
# gaussian to sample unknown word vectors
v = np.var(s,0)
m = np.mean(s,0)
...
print("Done with preprocesssing")
